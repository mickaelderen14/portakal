...
describe("SBPL compiler", () => {
  it("generates ESC A start and ESC Z end", () => {
    const output = label({ width: 40, height: 30 }).toSBPL();
  ...
  it("generates text with position and font", () => {
    const output = label({ width: 40, height: 30 })
      .text("Hello SATO", { x: 100, y: 50, size: 2 })
      .toSBPL();
  ...
  it("generates image with GM command", () => {
    ...
    const output = label({ width: 40, height: 30 }).image(bitmap, { x: 10, y: 10 }).toSBPL();
    expect(output).toContain("\x1bGM00002,FF00");
  });

  it("generates copies with ESC Q", () => {
    const output = label({ width: 40, height: 30, copies: 3 }).toSBPL();
    expect(output).toContain("\x1bQ3");
  });

  it("omits ESC Q for single copy", () => {
    const output = label({ width: 40, height: 30, copies: 1 }).toSBPL();
    expect(output).not.toContain("\x1bQ");
  });

  it("handles raw passthrough", () => {
    const output = label({ width: 40, height: 30 }).raw("\x1bKC1").toSBPL();
